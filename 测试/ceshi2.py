...
def show_plt():
    """遍历并绘制五组轨迹数据"""
    for i in range(1, 6):
        trail_list = eval(f"trail_{i}")  # 动态获取不同轨迹变量
        print(f"trail_{i}")
        ...
        plt.plot(t_trail, x_trail)  # 绘制 T-X 曲线
    plt.show()
...
def get_trail(move_distance, show=False):
    """
    根据传入的距离生成模拟的轨迹数据。
    :param move_distance: 运动的距离
    :param show: 是否展示轨迹波动曲线
    :return: 包含 [x, y, t] 坐标的轨迹列表
    """

    def __set_pt_time(_dist):
        """
        计算每个轨迹点的时间，并随机生成运动和停止阶段的时间点。
        """
        if _dist < 100:
            __need_time = int(random.uniform(500, 1500))  # 短距离所需时间
        else:
            __need_time = int(random.uniform(1000, 2000))  # 长距离所需时间

        __end_pt_time = []  # 结束阶段的时间点列表
        __move_pt_time = []  # 运动阶段的时间点列表
        __pos_z = []  # 所有时间点的最终列表

        # 计算运动阶段所需时间
        total_move_time = __need_time * random.uniform(0.8, 0.9)
        start_point_time = random.uniform(110, 200)  # 起始点时间
        __start_pt_time = [int(start_point_time)]  # 起始阶段时间列表

        sum_move_time = 0  # 运动时间累计

        _tmp_total_move_time = total_move_time
        while True:
            delta_time = random.uniform(15, 20)  # 随机时间间隔
    ...
    def __get_pos_y(point_count):
        """生成 Y 轴的轨迹点，用于模拟垂直方向的随机运动"""
        _pos_y = []
        start_y = random.randint(-1, 1)  # 起始 Y 坐标
        end_y = random.randint(-13, -5)  # 结束 Y 坐标
        y_values = np.linspace(start_y, end_y, point_count)  # 在起始和结束之间均匀分布
    ...
    # 获取时间列表和轨迹长度
    time_list = __set_pt_time(move_distance)
    trail_length = len(time_list)

    # 根据缓动函数生成 X 轴轨迹
    t = np.linspace(-0.5, 1, trail_length)  # 时间范围
    mult = move_distance / 7.59375  # 距离缩放系数
    x = [int(mult * (easeOutQuint(i) + 6.59375)) for i in t]  # 计算 X 坐标
    ...
    # 返回轨迹结果 [x, y, t]
    result = [[x[i], y[i], time_list[i]] for i in range(trail_length)]
    return result

def showeaseOutQuint(distance):
    """绘制缓动函数曲线，展示距离随时间变化的轨迹"""

    def func(x):
        return 1 - pow(1 - x, 5) + 6.59375

    print(func(-0.5), func(1))  # 打印函数在边界点的值
